...
function convertTreeToReact(
  rootElements,
  componentMap = {},
  rootComponent,
  propsForRootComponent
) {
  if (!rootComponent) {
    rootComponent = 'div';
  }

  const children = rootElements.map(child =>
    convertNodeToReact(child, componentMap)
  );
  return React.createElement(rootComponent, propsForRootComponent, children);
}
...
module.exports = convertTreeToReact;
